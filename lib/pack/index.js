...
const pack = (options) => {
	...
	Promise.all(promisesArray).then((a) => {
		...
		let platform = options.platforms[1]
		if(platform === 'chromecast' || platform === 'webostv') {
			let Platform = require('./'.concat(platform))
			let selectedPlatform = Platform(options)
			selectedPlatform.pack()
		}
		console.log('Goood')
	}).catch((err) => {
...
const symlinkFiles = (options) => {
	let currentDir = path.join(options.packer.output,options.platforms[1],'/')
	let targetDir = path.join(options.packer.input,options.platforms[0],'/')
	return new Promise((resolve, reject) => {
